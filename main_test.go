...
func TestStart(t *testing.T) {
	...
	terminal := Start(true, path + "/phishlets", true, true, cfgdir)
	if terminal == nil {
	...
	terminal.ProcessCommand("lures edit path 0 /inbound")
	test.assertLogContains("path = '/inbound'", "Can change lure path")
	
	terminal.ProcessCommand("lures edit redirect_url 0 http://example.com/authed")
